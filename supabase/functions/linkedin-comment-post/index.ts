...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { postUrn, commentText } = await req.json();

    if (!postUrn || !commentText) {
      return new Response(
        JSON.stringify({ error: "Missing postUrn or commentText" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        }
    ...
    if (!response.ok) {
      ...
      console.error("Failed to comment on post:", errorText);
      return new Response(
        JSON.stringify({ error: "Failed to comment on LinkedIn" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        }
