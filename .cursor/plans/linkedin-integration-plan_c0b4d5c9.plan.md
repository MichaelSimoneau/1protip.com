...
- Confirm/adjust edge functions to align with docs: ensure oauth exchange handles redirect URIs/scopes, profile fetch stores `profile_id`/avatar, and post sync filters `#1ProTip` with deduping; update Supabase SQL or function config if gaps exist.
- Wire client flows: Settings “Connect LinkedIn” triggers OAuth and shows connection status; Feed uses edge function to sync posts and handles errors gracefully; ensure routes under `app/(tabs)/settings.tsx` and `feed.tsx` align with hooks.
