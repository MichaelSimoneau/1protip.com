...
- New directive (2025-12-11): Ensure `expo-router` Head usage in `apps/free/app/_layout.tsx` follows the public API (import from `expo-router`, JSX children instead of string).
...
- New directive (2025-12-12): Fix LinkedIn auth hook: ensure `EXPO_PUBLIC_LINKEDIN_CLIENT_ID` is strictly required and validated (throws if missing) to prevent silent failures. Added `apps/free/app/auth/linkedin/callback.tsx` to handle OAuth redirect completion on web.
- New directive (2025-12-12): Add a hosting rewrite in `firebase.json` mapping `/logo.png` to `/assets/images/logo.png`. Manually copied `apps/free/assets/images/logo.png` to `apps/free/public/assets/images/logo.png` to ensure it exists in the web build output and the rewrite target is valid.
- New directive (2025-12-12): Configure CI (GitHub Actions) to inject the PRODUCTION LinkedIn Redirect URI (`https://free/auth/linkedin/callback`) and Client ID via job-level environment variables in `.github/workflows/deploy.yml`, ensuring production builds use the live domain instead of localhost.
...
- New directive (2025-12-12): Fix `react-native-pager-view` v7 web build error ("Importing native-only module") by creating a local web mock (`apps/free/mocks/react-native-pager-view.tsx` using `ScrollView`) and aliasing it in `metro.config.js` for the web platform.
- New directive (2025-12-12): Add root script `eas` aliased to `pnpm free exec eas` to allow running EAS CLI for the free app from the root.
- New directive (2025-12-13): Enforce strict LinkedIn env var naming: `EXPO_PUBLIC_LINKEDIN_CLIENT_ID`, `EXPO_SECRET_LINKEDIN_CLIENT_SECRET`, and `EXPO_SECRET_LINKEDIN_SERVICE_TOKEN`. Updated `functions/src/index.ts`, `scripts/sync-secrets.js`, and `env.example` to comply.
